...
In this model representation, $\Sigma_\varepsilon \in R^{KxK}$ and $\Sigma_\eta \in R^{KxK}$. Utilizing $\Sigma_\varepsilon$ and $\Sigma_\eta$ allows for estimation of correlation between cognitive tests. However, we must make restrictions on the possible values of the matrices for model identifiability. We propose the use of two different models, 
...
Both models of the correlation structure between the tests makes different assumptions regarding the underlying nature of the test. Model 1 assumes that any similarity we see in the cognitive scores for a given observation time is due to a correlated random effect on the day of the test. Model 2 makes the assumption that the underlying cognitive process is correlated between the tests across time.

For both models we propose the use of Bayesian estimation to estimate all parameters of interest.
...
\begin{enumerate}
  ...
  \item {Kalman Gain:} $\boldsymbol{K_ij} = \boldsymbol{\frac{\boldsymbol{P}_{ij|i(j-1)}}{\boldsymbol{P}_{ij|i(j-1)} + \sigma^2_\varepsilon}}$
  \item {Updated state estimate:} $\boldsymbol{\alpha}_{ij|ij} = \boldsymbol{\alpha}_{ij|i(j-1)} + \boldsymbol{K_ij} (\boldsymbol{\tilde y_ij- \alpha_{ij|i(j-1)}})$
  \item {Updated state covariance:} $\boldsymbol{P_{ij|ij}} = (1-\boldsymbol{K_ij})\boldsymbol{P_{ij|i(j-1)}}$
\end{enumerate}
...
On the other hand, if $\Sigma_\eta$ is non-diagonal as was shown in Model 2,
...
The covariance matrix $\Sigma_\varepsilon$ from model 1 and $\Sigma_\eta$ from model 2, both offer meaningful insight into underlying constructs of cognition. Additionally, these models allow for comparison of linear effects across different tests.
